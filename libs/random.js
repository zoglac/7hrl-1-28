...
// returns a value on a gaussian distribution
function gaussian(mean, stdev, min, max)
{
    return Math.min(Math.max(min, randstream.normal(mean, stdev)), max);
}

// Returns a value on a pareto distribution
function pareto(alpha, max)
{
    var retval = randstream.pareto(alpha);
    while (retval > max)
        retval = randstream.pareto(alpha);
    return retval;
}

function randbt(min, max)
{
    return Math.floor(randstream.random() * (max-min+1)) + min;
}
